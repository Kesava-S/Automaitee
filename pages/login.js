...
export default function Login({ user }) {
    ...
    const handleLogin = async (e) => {
        ...
        if (error) {
            ...
            // Redirect handled by auth state change in _app.js or useEffect above
            router.push('/')
        }
        setLoading(false)
    }
    ...
    return (
        <>
            <NextSeo title="Login" description="Login to your Kondamaal Automations account." />
            <div className="view active" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh' }}>
                <div className="card" style={{ maxWidth: '400px', width: '100%', padding: '2rem' }}>
                    ...
                    <p style={{ marginTop: '1.5rem', textAlign: 'center', fontSize: '0.9rem', color: '#6b7280' }}>
                        Don't have an account? <Link href="/signup" style={{ color: '#2563eb', textDecoration: 'none' }}>Sign up</Link>
                    </p>
                </div>
            </div>
        </>
